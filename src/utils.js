...
// Serializes an array of rows (each an array of cell values) into CSV text.
// Handles RFC-4180 escaping: quotes are doubled, cells containing the
// separator, quotes, or newlines are wrapped in double quotes.
export function csvSerialize(rows, separator = ',') {
...
export function downloadCsvFile(csvContent, filename) {
  ...
  const useBomSetting = localStorage.getItem("useBomForCsvExport");
  const BOM = useBomSetting === "false" ? "" : "\uFEFF";
...
/**
 * Capability-based record inspection helper.
 * Determines supported capabilities (View, Edit, Delete, Open) for a record.
 * @param {Object} record - Salesforce record object
 * @returns {Object} Record capabilities
 */
export function getRecordCapabilities(record) {
  const isAggregate = record?.attributes?.type === "AggregateResult";
  const hasId = Boolean(record?.Id || record?.id);
  const isDurableOnly = Boolean(record?.DurableId && !hasId);

  return {
    canView: !isAggregate && hasId,
    canEdit: !isAggregate && hasId,
    canDelete: !isAggregate && hasId,
    canOpenInSalesforce: !isAggregate && (hasId || isDurableOnly),
    isAggregate: isAggregate
  };
}

/**
 * SalesforceIdResolver - Centralized Salesforce ID validation, 15-to-18 character conversion, and object resolution
 */
export class SalesforceIdResolver {
  static isValidId(id) {
    return typeof id === "string" && /^[a-zA-Z0-9]{15}(?:[a-zA-Z0-9]{3})?$/.test(id);
  }

  static to18Char(id15) {
  ...
  static resolve(id, prefixMap = {}) {
    if (!this.isValidId(id)) {
      return { isValid: false, rawId: id };
    }
    const prefix = id.substring(0, 3);
    const normalizedId = id.length === 15 ? this.to18Char(id) : id;
    const knownStandardPrefixes = {
      "001": "Account", "003": "Contact", "005": "User", "006": "Opportunity",
      "00Q": "Lead", "500": "Case", "701": "Campaign", "00T": "Task", "00U": "Event"
    };
    const sObjectType = prefixMap[prefix] || knownStandardPrefixes[prefix] || "Unknown Object";
    ...
    return {
      ...
      isKnownObject: sObjectType !== "Unknown Object"
    };
  }
}
  ...
  try {
    ...
    function addEntity(
      ...
      if (entity) {
        ...
        if (api === "regularApi") {
          if (createable !== undefined) entity.regularCreateable = createable;
          if (deletable !== undefined) entity.regularDeletable = deletable;
          if (updateable !== undefined) entity.regularUpdateable = updateable;
        }
        if (api === "toolingApi") {
          if (createable !== undefined) entity.toolingCreateable = createable;
          if (deletable !== undefined) entity.toolingDeletable = deletable;
          if (updateable !== undefined) entity.toolingUpdateable = updateable;
        }
      } else {
        entity = {
          availableApis: [],
          ...
          regularCreateable: false,
          regularDeletable: false,
          regularUpdateable: false,
          toolingCreateable: false,
          toolingDeletable: false,
          toolingUpdateable: false,
        };
        if (api === "regularApi") {
          entity.regularCreateable = createable || false;
          entity.regularDeletable = deletable || false;
          entity.regularUpdateable = updateable || false;
        }
        if (api === "toolingApi") {
          entity.toolingCreateable = createable || false;
          entity.toolingDeletable = deletable || false;
          entity.toolingUpdateable = updateable || false;
        }

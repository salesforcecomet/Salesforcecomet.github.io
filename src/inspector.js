...
let storedApiVersion = localStorage.getItem("apiVersion");
if (!storedApiVersion || parseFloat(storedApiVersion) > 63.0) {
  storedApiVersion = defaultApiVersion;
  localStorage.setItem("apiVersion", defaultApiVersion);
}
export let apiVersion = storedApiVersion;

export let sessionError;
const clientId = "salesforce comet";
...
export let sfConn = {
  ...
  async rest(url, { logErrors = true, method = "GET", api = "normal", body = undefined, bodyType = "json", responseType = "json", headers = {}, progressHandler = null, useCache = true } = {}, rawResponse) {
    ...
    await new Promise((resolve, reject) => {
      ...
      xhr.ontimeout = () => {
        let err = new Error("Request timed out after 30 seconds. The Salesforce server may be slow or the query is too complex.");
    ...
    if (rawResponse) {
      ...
      if (!this._isRetrying401 && typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.sendMessage) {
        this._isRetrying401 = true;
        try {
          const freshSession = await new Promise((resolve) => {
            chrome.runtime.sendMessage({ action: 'getCookie', name: 'sid', url: window.location.href }, (resp) => {
              if (resp && resp.value) resolve(resp.value);
              else resolve(null);
            });
          });
          if (freshSession && freshSession !== this.sessionId) {
            this.sessionId = freshSession;
            this._isRetrying401 = false;
            return await this.rest(path, options);
          }
        } catch (retryErr) {
          // Ignore retry error and proceed to default error handler
        }
        this._isRetrying401 = false;
      }
      let error = (xhr.response && xhr.response.length > 0) ? xhr.response[0].message : "New access token needed";
